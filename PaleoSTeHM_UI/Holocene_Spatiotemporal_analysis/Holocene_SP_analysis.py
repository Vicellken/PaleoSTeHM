...
def load_rsl_data(csv_file_name):
    ...
    y = rsl.values[slip_index].astype(np.float64)
...
def implement_sp_gp_model(
    X_all, y, rsl_sigma, rsl_age_sigma, iteration=1000, seed=None
):
    """
    ...
    seed: An optional integer for random seed to ensure reproducibility. If None, no seed is set.
    ...
    optimized_params["regional_linear_temporal_kernel_variance"] = (
        regional_linear_temporal_kernel.variance.item()
    )

    # Extract optimized parameters for regional linear spatial kernel
    optimized_params["regional_linear_spatial_kernel_s_lengthscale"] = (
        regional_linear_spatial_kernel.s_lengthscale.item()
    )

    # Extract optimized parameters for regional non-linear temporal kernel
    optimized_params["regional_nl_temporal_kernel_variance"] = (
        regional_nl_temporal_kernel.variance.item()
    )
    optimized_params["regional_nl_temporal_kernel_lengthscale"] = (
        regional_nl_temporal_kernel.lengthscale.item()
    )

    # Extract optimized parameters for regional non-linear spatial kernel
    optimized_params["regional_nl_spatial_kernel_s_lengthscale"] = (
        regional_nl_spatial_kernel.s_lengthscale.item()
    )

    # Extract optimized parameters for local non-linear temporal kernel
    optimized_params["local_nl_temporal_kernel_variance"] = (
        local_nl_temporal_kernel.variance.item()
    )
    optimized_params["local_nl_temporal_kernel_lengthscale"] = (
        local_nl_temporal_kernel.lengthscale.item()
    )

    # Extract optimized parameters for local non-linear spatial kernel
    optimized_params["local_nl_spatial_kernel_s_lengthscale"] = (
        local_nl_spatial_kernel.s_lengthscale.item()
    )

    # Extract optimized parameters for site-specific white noise kernel
    optimized_params["sp_whitenoise_kernel_variance"] = (
        sp_whitenoise_kernel.variance.item()
    )
    ...
    # Extract optimized parameters for regional linear temporal kernel
    optimized_params["regional_linear_kernel_variance (m^2)"] = (
        regional_linear_temporal_kernel.variance.item()
    )

    # Extract optimized parameters for regional linear spatial kernel
    optimized_params["regional_linearl_kernel_spatil_lengthscale (km)"] = (
        regional_linear_spatial_kernel.s_lengthscale.item() * 6471
    )
    ...
    optimized_params["regional_nl_kernel_spatial_lengthscale (km)"] = (
        regional_nl_spatial_kernel.s_lengthscale.item() * 6471
    )
    ...
    optimized_params["local_nl_spatial_spatial_lengthscale (km)"] = (
        local_nl_spatial_kernel.s_lengthscale.item() * 6471
    )

    # Extract optimized parameters for site-specific white noise kernel
    optimized_params["sp_whitenoise_kernel_variance (m^2)"] = (
        sp_whitenoise_kernel.variance.item()
    )

    # Extract optimized parameters for white noise kernel
    optimized_params["whitenoise_kernel_variance (m^2)"] = (
        whitenoise_kernel.variance.item()
    )
...
def process_rsl_data(
    ...
    for i in range(len(all_region)):
        ...
        rsl_time, rsl_rate, rsl_rate_sd = PSTHM.post.cal_rate_var(
            test_age, y_var.detach().numpy(), y_mean.detach().numpy()
        )
        ...
        _, rsl_rate_global, rsl_rate_sd_global = PSTHM.post.cal_rate_var(
            test_age, global_var.detach().numpy(), global_mean.detach().numpy()
        )
        plt.plot(
        ...
        _, rsl_rate_local_nl, rsl_rate_sd_local_nl = PSTHM.post.cal_rate_var(
            test_age, local_nl_var.detach().numpy(), local_nl_mean.detach().numpy()
        )
        plt.plot(
            rsl_time,
...
def process_rsl_predictions_and_save(
    ...
    rsl_lon,
    gpr,
    test_time=np.arange(12000, 0, -1000),
    lat_step=0.3,
    lon_step=0.3,
